...
///////////////////////////////////////////////////////////////////////////////
integer DEFAULT_CHANNEL = -68192;                // channel used for saving
...
///////////////////////////////////////////////////////////////////////////////
string MSG_RESIDENTIAL_DOOR = "Open / Close Residential Door";
string MSG_LIGHTS_NOTECARD = "// Generated By % (Paste each line into a notecard named lights_yourscene)";
...
default 
{
    ...
    {      
        ...
        if (number == API_OPTION_DATA)
        {
            ...
            if( llToUpper(llList2String(command, 0)) == "DEFAULT" )
            {
                if (llToUpper(llList2String(command, 1)) == "DOOR" )
                {
                    if(TUTORIAL_CHATTY) llSay(0, MSG_RESIDENTIAL_DOOR);
                    llMessageLinked(LINK_THIS, 0, "RESIDENTIAL", NULL_KEY);
                    return;
                }                
            }            
            if( llToUpper(llList2String(command, 0)) == "NOTECARD" )
            {
                if (llToUpper(llList2String(command, 1)) == "LIGHTS" )
                {
                    if(TUTORIAL_CHATTY) llSay(0, strReplace(MSG_LIGHTS_NOTECARD,"%",PRODUCT_NAME));
                    llMessageLinked(LINK_THIS, 0, "LIGHTS", id);
                    return;
                }
                if (llToUpper(llList2String(command, 1)) == "TEXTURES" )
                {
                    if(TUTORIAL_CHATTY) llSay(0, strReplace(MSG_TEXTURES_NOTECARD,"%",PRODUCT_NAME));
            ...
            if ( llList2String(command, 0) == "SETTINGS")
            {
                ...
                if (llToUpper(llList2String(command, 1)) == "GROUP" )
                {
                    llMessageLinked(LINK_THIS, 0, "JOINGROUP:"+(string)id, id);
                    return;
                }
                if (llToUpper(llList2String(command, 1)) == "UPDATE" )
                {
                    llMessageLinked(LINK_THIS, 0, "UPDATE", id);
                    return;
                }                                
            } 
        }            
